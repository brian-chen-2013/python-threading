...
class RLock(_Verbose):
    ...
    def _acquire_restore(self, count_owner):
        ...
        self.__count = count_owner[0]
        self.__owner = count_owner[1]
        if __debug__:
...
class Condition(_Verbose):
    ...
    def _release_save(self):
        if hasattr(self.__lock, '_release_save'):
            return self.__lock._release_save()
        self.__lock.release()           # No state to save

    def _acquire_restore(self, x):
        if hasattr(self.__lock, '_acquire_restore'):
            return self.__lock._acquire_restore(x)
        self.__lock.acquire()           # Ignore saved state

    def _is_owned(self):
        # Return True if lock is owned by current_thread.
        # This method is called only if __lock doesn't have _is_owned().
        # RLock has a different _is_owned behavior from Lock.
        if hasattr(self.__lock, '_is_owned'): 
            return self.__lock._is_owned()
